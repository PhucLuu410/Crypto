...
void AES128_Add_Padding(uint8_t *Data, uint8_t Length)
{
    ...
    {
        Datapadding[Length + i] = 0x08;
    }

    for (int i = 0; i < 16; i++)
    {
        DataOut[i] = Datapadding[i];
    }
}
...
void AES128_MixColumb(uint8_t State[4][4])
{
    for (int j = 0; j < 4; j++)
    {
        uint8_t a[4];
